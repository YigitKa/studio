...
import { Phone, Mail, MapPin, Briefcase, GraduationCap, Wrench, User, Star, FolderGit2 } from 'lucide-react';
...
export function ResumePreviewModern() {
  ...
  const leftColumnSections: ResumeSection[] = ['profile', 'skills', 'education', 'customSections'];
  const rightColumnSections: ResumeSection[] = ['summary', 'experience', 'projects'];
  ...
  const sectionComponents: Record<string, React.ReactNode> = {
    ...
    projects: settings.showProjects && projects.length > 0 ? (
      <section>
        <h3 className="text-base font-semibold text-[#0b2540] border-l-4 border-[#6b7bd8] pl-1.5 mb-1.5 flex items-center gap-2">
          <FolderGit2 size={16} /> {t('projects')}
        </h3>
        <div className="space-y-3">
          {projects.map(proj => (
            <div key={proj.id}>
              <div className="text-sm text-[#3b4a6b] font-semibold">{proj.name}</div>
  ...
  const visibleLeftSections = sections.filter(sectionId => 
    leftColumnSections.includes(sectionId as ResumeSection) && sectionComponents[sectionId]
  );

  const visibleRightSections = sections.filter(sectionId =>
    rightColumnSections.includes(sectionId as ResumeSection) && sectionComponents[sectionId]
  );
